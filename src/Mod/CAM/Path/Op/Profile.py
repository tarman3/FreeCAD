...
class ObjectProfile(PathAreaOp.ObjectOp):
    ...
    def _processOpenWireNG(self, obj, base, wire):
        ...
        # z = wire.BoundBox.ZMax
        # zs = (z, z + self.tol, z - self.tol)
        # slices = [w for s in self.solids for z in zs for w in s.slice(FreeCAD.Vector(0, 0, 1), z)]
        # for w in slices:
        #     w.translate(FreeCAD.Vector(0, 0, wire.BoundBox.ZMax - w.BoundBox.ZMax))
        # shape = Part.Compound(slices)

        # vfaces = [f for s in self.solids for f in s.Faces if Path.Geom.isVertical(f)]
        # shape = Part.Compound(vfaces)
        # Part.show(shape)

        solids = Part.Compound(self.solids)
        cutting_plane = Path.Geom.makeBoundBoxFace(Part.Compound(solids).BoundBox, 10)
        cutting_plane.translate(
            FreeCAD.Vector(0, 0, wire.BoundBox.ZMax - cutting_plane.BoundBox.ZMax)
        )
        # Part.show(cutting_plane)
        shape = solids.section(cutting_plane)
        # Part.show(section)
        ...
        for po in passOffsets:
            ...
            owire = Path.Op.Util.offsetWire(wire, base.Shape, po, True, tolerance=self.tol)
