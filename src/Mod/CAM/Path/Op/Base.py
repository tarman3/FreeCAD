...
class ObjectOp:
    ...
    def checkDepths(self, obj):
        ...
        isValid = True
        if FeatureDepths & self.opFeatures(obj):
            if obj.FinalDepth > obj.StartDepth and not Path.Geom.isRoughly(
                obj.FinalDepth.Value, obj.StartDepth.Value
            ):
                Path.Log.error(translate("CAM_Operation", "FinalDepth above StartDepth\n"))
                isValid = False
            if obj.StartDepth > obj.SafeHeight and not Path.Geom.isRoughly(
                obj.StartDepth, obj.SafeHeight
            ):
                Path.Log.error(translate("CAM_Operation", "StartDepth above SafeHeight\n"))
                isValid = False
            if obj.SafeHeight > obj.ClearanceHeight and not Path.Geom.isRoughly(
                obj.SafeHeight, obj.ClearanceHeight
            ):
