...
class MoveArc(Instruction):
    ...
    def anglesOfTangents(self):
        ...
        Path.Log.debug(
            f"MoveArc: arc direction={math.degrees(arc_dir):.2f}°, tangent angles t0={math.degrees(t0):.2f}°, t1={math.degrees(t1):.2f}°"
        )
...
class Maneuver(object):
    ...
    def FromPath(cls, path):
        ...
        Path.Log.debug(
            f"Maneuver.FromPath: creating maneuver from path with {len(path.Commands)} commands"
        )
...
class DressupTangential(object):
    ...
    def __init__(self, obj, base, job):
        ...
        Path.Log.debug(
            f"DressupTangential.__init__: properties set, TangentialAngle={obj.TangentialAngle}"
        )
        Path.Log.info(
            f"DressupTangential initialized for base: {base.Label if hasattr(base, 'Label') else 'Unknown'}"
        )
                    ...
                    if cmd.Name in Constants.GCODE_MOVE_ALL:
                        ...
                        if hasattr(cmd, "Parameters"):
                            ...
                            if old_position != current_position:
                                Path.Log.debug(
                                    f"  Position updated: {old_position} -> {current_position}"
                                )

                obj.Path = Path.Path(output_commands)
                Path.Log.info(
                    f"Tangential dressup completed: processed {len(output_commands)} commands ({move_count} moves)"
                )
...
def Create(base, name="TangentialDressup"):
    ...
    Path.Log.debug(
        f"Creating tangential dressup for base: {base.Label if hasattr(base, 'Label') else base}"
    )
